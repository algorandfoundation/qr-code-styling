...
describe("Test QRCodeStyling class", () => {
    ...
    it("The README example should work correctly", (done) => {
        const expectedQRCodeFile = fs.readFileSync(path.resolve(__dirname, "../assets/test/image_from_readme.png"), "base64");
    ...
    it("Compatible with jsdom", (done) => {
        const expectedQRCodeFile = fs.readFileSync(path.resolve(__dirname, "../assets/test/image_from_readme.svg"), "base64");
        const qrCode = new QRCodeStyling({
            jsdom: JSDOM,
            type: "svg",
            width: 300,
